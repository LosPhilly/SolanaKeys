...
echo " Running on LeoV1.0"
echo "========================================================================"
echo " SOLANA KEY DEPLOYMENT COMMAND CENTER (ED25519)"
echo "========================================================================"
echo ""
echo " DESCRIPTION:"
...
echo " pipeline and relays the telemetry back to your C2 infrastructure."
echo ""
echo " HOW TO USE:"
...
if [ "$MODE_SELECTION" == "1" ]; then
    echo -e " ${CYAN}[SYSTEM] Compiling strict Base58 strings into silicon gates...${NC}"
    
    # Inject string targets directly into the NVCC compiler stream
    nvcc -arch=sm_86 -O3 \
        -DDYNAMIC_MODE=0 \
        -DTARGET_PREFIX_STR=\"${PREFIX_INPUT}\" \
        -DTARGET_SUFFIX_STR=\"${SUFFIX_INPUT}\" \
        solana_engine_master.cu -o solana_engine

    echo -e " ${GREEN}[SYSTEM] Build complete. Launching ballistic engine...${NC}"
    echo "========================================================================"
    sleep 1
    ...
    echo -e " ${CYAN}[SYSTEM] Compiling dynamic routing architecture...${NC}"
    
    nvcc -arch=sm_86 -O3 -DDYNAMIC_MODE=1 solana_engine_master.cu -o solana_engine
    
    echo -e " ${GREEN}[SYSTEM] Build complete. Launching dynamic engine via CLI arguments...${NC}"
    echo "========================================================================"
    sleep 1

/**
 * ============================================================================
 * PROJECT CODE-NAME: SOLANA_VANITY_MASTER_CORE
...
int main(int argc, char **argv) {
    ...
    cudaMalloc((void**)&d_found_secret, 4 * sizeof(uint64_t));
    cudaMalloc((void**)&d_found_pubkey, 32 * sizeof(uint8_t));
    cudaMalloc((void**)&d_found_address, 44 * sizeof(char));
    ...
    int total_filter_chars = 0;
        ...
        cudaMemcpyToSymbol(D_TARGET_SUFFIX, argv[2], suffix_len * sizeof(char));
        cudaMemcpyToSymbol(D_TARGET_SUFFIX_LEN, &suffix_len, sizeof(int));

        printf("Execution Profile : \033[1;36m[PHANTOM COMPATIBLE / DYNAMIC CACHE]\033[0m\n");
    #else
    ...
    while (true) { 
        loop_iteration++;
        cudaMemset(d_match_flag, 0, sizeof(int));
        
        solana_vanity_kernel<<<blocks_per_grid, threads_per_block>>>(base_seed, d_found_secret, d_found_address, d_found_pubkey, d_match_flag);
        cudaDeviceSynchronize();

        cudaMemcpy(&h_match_flag, d_match_flag, sizeof(int), cudaMemcpyDeviceToHost);

        if (h_match_flag == 1) {
            cudaMemcpy(h_found_secret, d_found_secret, 4 * sizeof(uint64_t), cudaMemcpyDeviceToHost);
            cudaMemcpy(h_found_address, d_found_address, 44 * sizeof(char), cudaMemcpyDeviceToHost);
            cudaMemcpy(h_found_pubkey, d_found_pubkey, 32 * sizeof(uint8_t), cudaMemcpyDeviceToHost);
    ...
    cudaFree(d_match_flag);
    cudaFree(d_found_secret);
